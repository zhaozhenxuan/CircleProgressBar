...
public class CircleProgressBar extends View {
    ...
    private interface onMyAnimatorUpdateListener {
    ...
    private static class MyAnimatorUpdateListener implements ValueAnimator.AnimatorUpdateListener {
        ...
        private WeakReference<onMyAnimatorUpdateListener> listenerWeakReference;

        MyAnimatorUpdateListener(onMyAnimatorUpdateListener listener) {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            if (listenerWeakReference.get() != null) {
                listenerWeakReference.get().onAnimatorUpdate(animation);
            }
        }
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mAnimatorListener = new MyAnimatorUpdateListener(new onMyAnimatorUpdateListener() {
            @Override
            public void onAnimatorUpdate(ValueAnimator animation) {
                if (animation == mProgressAnimator) {
                    mAnimatorProgress = (int) animation.getAnimatedValue();
                    mAnimatorProgressAngle = mSweepAngle * ((float) mAnimatorProgress / mMax);
                    invalidate();
                } else if (animation == mLoadingAnimator) {
                    mLoadingOffset = (float) animation.getAnimatedValue();
                    invalidate();
                }
            }
        });
